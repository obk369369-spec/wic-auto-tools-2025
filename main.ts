...
// Deno Deploy 표준 API만 사용. 외부 모듈 없음.

const DATA_ORIGIN =
  Deno.env.get("WIC_DATA_ORIGIN") ??
  "https://wic-auto-tools-2025-data.obk369369-spec.deno.net"; // 외부 데이터 원본 서버 지정
...
async function post(path: string): Promise<boolean> {
  try {
    const r = await fetch(`${DATA_ORIGIN}${path}`, {
      method: "POST",
      cache: "no-store",
    });
    return r.ok;
...
async function getLatest(): Promise<{ ok: boolean; status: number; data: any | null }> {
  try {
    const r = await fetch(`${DATA_ORIGIN}/export/latest.json`, { cache: "no-store" });
...
function toCompact(raw: any, origin: string) {
  ...
  const agg = new Map<
    string,
    { cnt: number; active: number; standby: number; blocked: number; done: number; sum: number }
  >();
  for (const r of rows) {
    const g = r["그룹"];
    if (!agg.has(g))
      agg.set(g, { cnt: 0, active: 0, standby: 0, blocked: 0, done: 0, sum: 0 });
    const a = agg.get(g)!;
    a.cnt++;
    a.sum += toInt(r["진행률(%)"]);
    const s = r["상태"];
    if (s === "active") a.active++;
    else if (s === "standby") a.standby++;
    else if (s === "blocked") a.blocked++;
    else if (s === "done") a.done++;
  }
  ...
  return {
    ...
    stalled,
    warn: stalled
      ? "데이터가 90분 이상 갱신되지 않았습니다. 파이프라인 정지(stalled) 가능성."
      : null,
    rows,
    group_summaries,
  };
...
Deno.serve(async (req) => {
  try {
    ...
    if (pathname === "/report/live") {
      ...
      const compact = toCompact(latest.data, DATA_ORIGIN);
      return json(compact);
    }
